...
| 公平性 | 双方使用同一 mock upstream、同一逻辑模型（`mock-model`，provider 前缀按实现不同）、同一压测客户端 |

## 2. 结果总览

> **2026-09-20 重跑**（当前构建：Responses/provider protocol 修复后的版本）。基准实例使用独立端口，
> **绝不占用线上服务端口 20128**。原始数据：`/tmp/omni-bench-results6/`。

### 内存（RSS，进程树合计）

| 状态 | omniroute-rust | 原版（TS） | 倍数 |
|---|---|---|---|
| 空闲（启动后） | **9.4 MB** | 718 MB | **约 76×** |
| `/healthz` 16 并发 | 12.2 MB | 957 MB | 约 78× |
| chat JSON 16 并发 | 24.9 MB | 983 MB | 约 39× |
| chat JSON 64 并发 | 30.8 MB | 1.10 GB | 约 36× |
| SSE 64 并发 | 31.0 MB | 1.26 GB | 约 41× |

峰值：Rust **31.0 MB**，原版 **1.26 GB**。

### 并发吞吐（同一 mock upstream，两侧错误数均为 0）

| 场景 | 并发 | omniroute-rust | 原版 | 倍数 |
|---|---|---|---|---|
| `GET /healthz`（纯网关开销） | 16 | **12,328 rps** | 299 rps | 约 41× |
| `GET /healthz`（纯网关开销） | 64 | **16,032 rps** | 373 rps | 约 43× |
| chat JSON（代理） | 16 | **2,147 rps** | 19 rps | 约 114× |
| chat JSON（代理） | 64 | **3,381 rps** | 32 rps | 约 106× |
| chat SSE 流式 | 16 | **312 rps** | 27 rps | 约 12× |
| chat SSE 流式 | 64 | **1,077 rps** | 32 rps | 约 34× |
...
| JSON 代理 c16 | 5 / 14 | 748 / 1,309 |
| JSON 代理 c64 | 13 / 32 | 1,792 / 3,084 |
...
- 内存：空闲 **约低 76×**（9.4 MB vs 718 MB），满负载 **约低 36–41×**（31.0 MB vs 1.10–1.26 GB）
- 吞吐：JSON 代理 **约 106–114×**，纯网关路径 **约 41–43×**，SSE 流式 **约 12–34×**
- 延迟：JSON 代理 p50 为 5–13ms（原版为 748–1,792ms）；64 并发下 p99 为 32ms（原版 3,084ms）
...
上述数字使用同一套 `examples/loadgen.rs`：先校验基准端口空闲，在 20129 压 Rust、
