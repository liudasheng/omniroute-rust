...
|---|---|---|---|
| Idle (after boot) | **7.0 MB** | ~850 MB | **~120×** |
| Under load (16-concurrent JSON) | 22 MB | ~1.08 GB | ~49× |
| Under load (64-concurrent JSON) | 23 MB | ~1.12 GB | ~49× |
| Under load (SSE, 64 concurrent) | 23 MB | ~1.20 GB | ~53× |

> The original is the full production stack: Next.js standalone server +
> instrumentation + model-catalog/quota caches + per-request SQLite history
> writes; idle is ~850 MB (CLI launcher ~130 MB + Next server ~600 MB +
> resident esbuild helper), growing past 1 GB under traffic. The Rust version
> is a single process: 7 MB idle, ~23 MB at full load.

### Concurrency throughput (same mock upstream, zero errors)

| Scenario | Concurrency | omniroute-rust | Original | Ratio |
|---|---|---|---|---|
| `/healthz` (pure gateway) | 16 | **3,050 rps** | 35 rps | ~87× |
| `/healthz` (pure gateway) | 64 | **4,429 rps** | 38 rps | ~115× |
| chat JSON (proxy) | 16 | **3,928 rps** | 32 rps | ~123× |
| chat JSON (proxy) | 64 | **5,419 rps** | 32 rps | ~169× |
| chat SSE streaming | 16 | **317 rps** | 24 rps | ~13× |
| chat SSE streaming | 64 | **1,131 rps** | 21 rps | ~53× |
...
| JSON proxy c16 | 3 / 5 | 417 / 656 |
| JSON proxy c64 | 7 / 14 | 2,056 / 2,683 |
| SSE c16 | 47 / 53 | 537 / 761 |
| SSE c64 | 51 / 59 | 3,215 / 4,230 |

> Note: in SSE mode both sides are bounded by the mock's 6-chunk sequence (6
> events per request); rps counts complete SSE transactions, not single
> chunks. The Rust 1,131 rps at c64 ≈ 6,800 chunk frames per second.
...
```bash
# 1) Build the mock upstream and the load generator
...
# 3a) Rust gateway (rate limits raised)
...
# 3b) Original (npm i -g omniroute; raise limits via PATCH /api/resilience)
# 4) Load test
...
# 5) Memory: sample the process tree mid-load with `ps -o rss=`
```

Raw results (JSON / RSS samples) were captured under `/tmp/bench-results2/`
(runtime snapshot of this benchmark run).
